package com.novexa.platform.feature.dashboard.presentation.component
...
import com.novexa.platform.core.ui.base.createAdapter
import com.novexa.platform.databinding.ItemCampaignBannerBinding
...
class BannerView @JvmOverloads constructor(
    ...
    private val adapter by lazy {
        createAdapter<PromotionBannerResponse, ItemCampaignBannerBinding>(ItemCampaignBannerBinding::inflate) { item, _ ->
            ivIcon.loadImage(item.activityPicUrl)
        }.apply {
            setOnItemClickListener { item, _ ->
                if (!item.activityH5Url.isNullOrBlank()) {
                    ContentBrowserActivity.launch(context, item.name ?: "", item.activityH5Url)
                }
            }
        }
    }

    private var data: List<PromotionBannerResponse> = emptyList()
    ...
    fun setData(list: List<PromotionBannerResponse>) {
        data = list
        if (list.isEmpty()) return

        // build virtual list (add fake head/tail)
        val loopList = mutableListOf<PromotionBannerResponse>()
    ...
    private fun setupIndicators(count: Int) {
        ...
        repeat(count) {
            val dot = View(context).apply {
                ...
                val margin = 3.dp
                layoutParams = LinearLayout.LayoutParams(16.dp, 5.dp).apply {
                    leftMargin = margin
                    rightMargin = margin
                }
            }
