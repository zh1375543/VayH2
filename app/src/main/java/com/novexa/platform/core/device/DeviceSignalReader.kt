package com.novexa.platform.core.device
...
object DeviceSignalReader {
    ...
    private val packageManager: PackageManager = MainApplication.appContext.packageManager

    fun getCurrentAppInstalledTime(): Long = try {
        packageManager.getPackageInfo(MainApplication.appContext.packageName, 0).firstInstallTime
    ...
    suspend fun getNetworkInfo(): JSONObject = withContext(Dispatchers.IO) {
        JSONObject().apply {
            runCatching {
                val connectivityManager = MainApplication.appContext.getSystemService(
                    Context.CONNECTIVITY_SERVICE
                ) as ConnectivityManager
                val wifiManager = MainApplication.appContext.applicationContext.getSystemService(
    ...
    private fun getSimCardCount(): Int {
        ...
        return runCatching {
            SubscriptionManager.from(MainApplication.appContext).activeSubscriptionInfoCount
        }.getOrDefault(0)
    }
    ...
    private fun getSimCardsJson(): JSONArray {
        ...
        val subscriptions = SubscriptionManager.from(MainApplication.appContext).activeSubscriptionInfoList
            ?: return JSONArray()
    ...
    private fun telephonyManager(): TelephonyManager = MainApplication.appContext.getSystemService(
        Context.TELEPHONY_SERVICE
    ) as TelephonyManager

    private fun bluetoothAdapter(): BluetoothAdapter? = (MainApplication.appContext.getSystemService(
    ...
    private fun getBluetoothMac(): String = if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
        Settings.Secure.getString(MainApplication.appContext.contentResolver, "bluetooth_address") ?: ""
    } else {
        ""
    }
}
