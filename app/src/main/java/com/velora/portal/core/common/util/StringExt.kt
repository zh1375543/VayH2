...
/** Masks a phone number while preserving enough digits for recognition. */
fun String.maskPhoneNumber(): String = when {
    isBlank() -> "-"
    length <= 4 -> this
    length <= 7 -> "${first()}****${last()}"
...
fun String?.showToastMessage() {
    ...
    if (now - lastToastTime >= INTERVAL) {
        lastToastTime = now
        Toast.makeText(MainApplication.appContext, this, Toast.LENGTH_SHORT).show()
    }
}
