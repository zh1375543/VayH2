 ...
 *
 * Supported XML attributes (declare-styleable [R.styleable.StatefulActionButton]):
 *
 ...
 */
class StatefulActionButton @JvmOverloads constructor(
    context: Context,
     ...
     *
     * When no custom color is provided, only the variant is changed and the current colors are
     * preserved. When any custom color is provided, unspecified colors fall back to their
     * variant defaults.
     */
    fun updateAppearance(
        ...
        val hasCustomColors = solidColor != null || strokeColor != null || textColor != null
        if (!hasCustomColors) {
    ...
    private fun parseAttributes(attrs: AttributeSet?, defStyleAttr: Int) {
        val typedArray = context.obtainStyledAttributes(
            attrs,
            R.styleable.StatefulActionButton,
            defStyleAttr,
            0,
        )
        variant = typedArray.getInt(R.styleable.StatefulActionButton_actionButtonVariant, VARIANT_FILLED)
        radius = typedArray.getDimension(R.styleable.StatefulActionButton_buttonRadius, radius)
        solidColor = typedArray.getColorOrNull(R.styleable.StatefulActionButton_buttonSolidColor)
        pressedColor = typedArray.getColorOrNull(R.styleable.StatefulActionButton_buttonPressedColor)
        disabledColor = typedArray.getColorOrNull(R.styleable.StatefulActionButton_buttonDisabledColor)
        strokeWidth = typedArray.getDimensionPixelSize(
            R.styleable.StatefulActionButton_buttonStrokeWidth,
            strokeWidth,
        )
        strokeColor = typedArray.getColorOrNull(R.styleable.StatefulActionButton_buttonStrokeColor)
        normalTextColor = typedArray.getColorOrNull(R.styleable.StatefulActionButton_buttonTextColor)
        disabledTextColor = typedArray.getColorOrNull(R.styleable.StatefulActionButton_buttonDisabledTextColor)
        disabledAlpha = typedArray.getFloat(
            R.styleable.StatefulActionButton_buttonDisabledAlpha,
