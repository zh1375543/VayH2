...
fun trackEvent(event: String, params: Map<String, Any?>) {
    ...
    runCatching {
        AppsFlyerLib.getInstance().logEvent(MainApplication.appContext, event, appsFlyerPayload)
    }.onFailure {
        LogUtil.e("AppsFlyer track failed: ${it.message}")
    }

    runCatching {
        FirebaseAnalytics.getInstance(MainApplication.appContext).logEvent(event, params.toFirebaseBundle())
    }.onFailure {
