...
fun Uri.uriToPart(
    ...
    val inputStream = MainApplication.appContext.contentResolver.openInputStream(this)

    val file =
        File(MainApplication.appContext.cacheDir, "${partName}_${System.currentTimeMillis()}.jpg")
...
fun isNetworkAvailable(): Boolean {
    val manager =
        MainApplication.appContext.applicationContext.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    return manager.activeNetworkInfo?.isAvailable == true
}

@SuppressLint("Range")
fun Uri.getContactInfo(action: (String, String) -> Unit) {
    val cursor = MainApplication.appContext.contentResolver?.query(
        this,
        arrayOf(
