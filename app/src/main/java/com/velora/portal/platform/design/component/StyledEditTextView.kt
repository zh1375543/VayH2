...
import com.velora.portal.platform.design.component.shape.TextStateController
...
class StyledEditTextView @JvmOverloads constructor(
    ...
    /** Updates the default and focused border color without changing the selected error border. */
    fun setStrokeColor(@ColorInt color: Int) {
        shapeAttributes = shapeAttributes.copy(
            appearance = shapeAttributes.appearance.copy(
                normal = shapeAttributes.appearance.normal.copy(strokeColor = color),
                focused = shapeAttributes.appearance.focused.copy(strokeColor = color),
            ),
        )
        backgroundController.apply(shapeAttributes.appearance)
        backgroundController.updateBounds(width, height)
    ...
    /** Updates only the border color displayed while this input has focus. */
    fun setFocusedStrokeColor(@ColorInt color: Int) {
        shapeAttributes = shapeAttributes.copy(
            appearance = shapeAttributes.appearance.copy(
                focused = shapeAttributes.appearance.focused.copy(strokeColor = color),
            ),
